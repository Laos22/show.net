...
if ( ! function_exists( 'scllgn_init' ) ) {
	function scllgn_init() {   
		...
		if ( isset( $_GET['provider'] ) && in_array( $_GET['provider'], array( 'google', 'facebook', 'twitter', 'linkedin', 'instagram' ) ) ) {
...
if ( ! function_exists( 'scllgn_get_default_options' ) ) {
	function scllgn_get_default_options( $is_network_admin = false ) {
		...
		$default_options = array(
			...
			'instagram_button_name'                 => sprintf( __( 'Sign in with %s', 'social-login-bws' ), 'Instagram' ),
...
if ( ! function_exists( 'scllgn_get_button' ) ) {
	function scllgn_get_button( $provider = '', $echo = false ) {
		...
		if ( 'instagram' == $provider ) {
			$authUrl = wp_login_url() . "?provider=instagram";
			$dashicon_for_button = 'bws-icons';
			$button_html = $scllgn_options['button_display_instagram'];
			$button_text = $scllgn_options['instagram_button_name'];
			if ( isset( $_GET['provider'] ) && 'instagram' == $_GET['provider'] ) {
				scllgn_social_client( 'Instagram' );
			}
		}
	...
	function scllgn_user_profile() {
		...
		$fields = array(
			...
			'instagram'       => array(
				'description'   => sprintf(
					$description_string,
					__( 'existing login of Instagram account', 'social-login-bws' )
				),
				'field_type'    => 'text'
			)
		);
		...
		if ( 0 != $scllgn_options['google_is_enabled'] || 0 != $scllgn_options['facebook_is_enabled'] || 0 != $scllgn_options['twitter_is_enabled'] || 0 != $scllgn_options['linkedin_is_enabled'] || 0 != $scllgn_options['instagram_is_enabled'] ) { ?>
		...
		foreach ( $scllgn_providers as $provider => $provider_name ) {
			if ( isset( $_POST['scllgn_' . $provider . '_login'] ) ) {
				...
				if ( ! empty( $provider_login ) ) {
					if ( is_email( $provider_login ) || 'instagram' == $provider ) { /* preg_match is used for PHP versions older than 5.3 */
